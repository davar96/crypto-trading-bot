...
def create_exchange(use_testnet=False):
    ...
    config = {
        ...
        "options": {
            "defaultType": "future",  # Required for funding rates
        },
    }
    ...
    if use_testnet:
        ...
        # For production, CCXT can use keys from environment variables by default
        # if named BINANCE_API_KEY and BINANCE_SECRET_KEY, but explicit is safer.
        config.update(
            {
...
def fetch_funding_rate_history(exchange, symbol, start_dt):
    ...
    # For funding rates, the symbol needs to be in perpetual format, e.g., 'BTC/USDT:USDT'
    perp_symbol = f"{symbol.split('/')[0]}/USDT:USDT"

    since = int(start_dt.timestamp() * 1000)
    all_rates = []
    limit = 1000  # Max limit for funding history is often lower, but CCXT handles it
    ...
    while True:
        try:
            ...
            if not len(rates):
                break
            # The 'since' for the next request is the timestamp of the last item
    ...
    # We only need timestamp and the fundingRate itself
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "data_type", choices=["ohlcv", "funding"], help="The type of data to download ('ohlcv' or 'funding')."
    )
    ...
    if args.data_type == "ohlcv":
        # For OHLCV, we still fetch multiple timeframes
        fetch_ohlcv_data(exchange_instance, args.symbol, "1h", start_date)
        fetch_ohlcv_data(exchange_instance, args.symbol, "5m", start_date)
